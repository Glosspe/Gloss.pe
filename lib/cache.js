...
const MAX_MEMORY_ENTRIES = 100; // Techo duro para evitar OOM en contenedores de 512MB
...
class HybridCache {
    ...
    if (redisUrl) {
      ...
      try {
        ...
        this.redis.on('connect', () => {
          ...
          // Latido de Keep-Alive recurrente (Keep-Warm) para mantener despierta la base de datos de Redis
            ...
            this.pingInterval = setInterval(async () => {
              if (this.isRedisConnected && this.redis) {
                try {
                  ...
                  console.warn('[Cache Redis] Latido de Keep-Alive fallido:', pingErr.message);
                }
              }
            }, 120000); // Ejecutar cada 2 minutos (120 segundos)
            
            // Unref permite que Next.js finalice las fases de compilación y empaquetado sin quedarse colgado
  ...
  async set(key, value, ttlSeconds = 60) {
    ...
    // Modo Memoria Local — con límite de entradas para evitar heap overflow
    // Primero, limpiar entradas expiradas
    const now = Date.now();
    if (this.memoryCache.size > MAX_MEMORY_ENTRIES * 0.8) {
      for (const [k, v] of this.memoryCache) {
        if (now > v.expiresAt) this.memoryCache.delete(k);
      }
    }
    // Si aún excede el límite, eliminar las más antiguas (FIFO)
    if (this.memoryCache.size >= MAX_MEMORY_ENTRIES) {
      const keysToDelete = Array.from(this.memoryCache.keys()).slice(0, Math.floor(MAX_MEMORY_ENTRIES * 0.3));
